...
const Infrastructure = () => {
  ...
  const [upgrades, setUpgrades] = useState(state.infraStructureUpgrade);

  const handleUpgrade = (index) => {
    if (upgrades[selectedFactor][index].percent < 100 && state.budget >= upgrades[selectedFactor][index].cost) {
...
useEffect(()=>{
    dispatch({type:"INF_UPGRADE",payload:upgrades});
  },[upgrades]);
  return (
    <div className="action-main-container">
      <div className="action-head-nav">
        <div className="action-type-name">Infrastructure</div>
